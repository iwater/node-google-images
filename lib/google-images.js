...
module.exports = {
  search: function(query, options) {
    ...
    if (options.size == null) options.imgsz = 'medium';
    return request("http://ajax.googleapis.com/ajax/services/search/images?v=1.0&q=" + (query.replace(/\s/g, '+')) + "&start=" + options.page + "&imgsz=" + options.size, function(err, res, body) {
